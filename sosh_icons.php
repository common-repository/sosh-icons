...
Text Domain: sosh-icons
    ...
    Copyright (C) 2013-2016, twentyZen GmbH - contact@twentyZen.com
...
<form method="post" action="options.php">
    ...
    <p>
       <?php
        ...
        mit webZunder aus</b>. Ganz einfach <b>14 Tage unverbindlich testen </b> und das eigene Online Marketing anheizen.','sosh-icons');
        echo '<br><br>';
...
function soshicons_plugin_init() {
    ...
    add_settings_field('wa_setting', 'WhatsApp', 'wa_setting_check', 'soshicons_plugin', 'soshicons_plugin_main');
...
add_settings_field('cta_setting', 'Call to Action', 'cta_setting_check', 'soshicons_plugin', 'soshicons_plugin_main');
    ...
    add_settings_field('excerpt_show',  __('Excerpt','sosh-icons'), 'excerpt_check', 'soshicons_show_plugin', 'soshicons_plugin_show');
    ...
    $settarray=array('twt_setting','fb_setting','gplus_setting','xi_setting','link_setting', 'pin_setting', 'tl_setting', 'vk_setting', 'wa_setting' ,'mail_setting', 'position_setting','cta_setting');
    ...
    $showarray=array('posts_show','pages_show','frontpage_show','archives_show','excerpt_show');
...
function wa_setting_check(){
    $options=get_option('soshicons_options');
   echo '<li><i class="fa fa-whatsapp fa-2x option"></i>';
   echo '<input type="checkbox" id="wa_setting" name="soshicons_options[wa_setting]" value="1"'. checked( "1", $options['wa_setting'], false ).' /><span>'.__('nur mobil sichtbar','sosh-icons').'</span>';
    echo "</li>";
}
...
function cta_setting_check(){
	$options=get_option('soshicons_options');
	echo '<input type="text" id=“cta_setting” name="soshicons_options[cta_setting]" value="'.$options['cta_setting'].'"/>';
...
function excerpt_check(){
   $showoptions=get_option('soshicons_show_options');
   echo '<input type="checkbox" id="excerpt_show" name="soshicons_show_options[excerpt_show]" value="1"'. checked( "1", $showoptions['excerpt_show'], false ).' />';
}
...
function twt_related_check(){
    $advoptions=get_option('soshicons_advanced_options');
         ...
         echo '<span class="description">'.__('Welcher Twitteraccount soll nach dem Teilen, deinem Leser empfohlen werden. Bitte ohne @ Zeichen. Hier ein <a href="https://g.twimg.com/dev/documentation/image/tweet-intent-posted.png">Beispiel-Bild wie das dann aussieht</a>.','sosh-icons').'</span>';
...
$options = get_option('soshicons_options');
    ...
    if(isset($input['wa_setting'])&& $input['wa_setting']!=""){
        $options['wa_setting'] = $input['wa_setting'];
    }else{
        $options['wa_setting'] = "0";
    }
    ...
    if(isset($input['cta_setting']) && (($input['cta_setting']!="")||($input['cta_setting']!="0"))){
        $options['cta_setting'] = $input['cta_setting'];
    }else{
        $options['cta_setting'] = "Ganz einfach weitersagen!";
    }
...
$showoptions = get_option('soshicons_show_options');
    ...
    if(isset($input['excerpt_show'])&& $input['excerpt_show']!=""){
       $showoptions['excerpt_show'] = $input['excerpt_show'];
    }else{
        $showoptions['excerpt_show'] = "0";
    }
...
function soshicons_displayicons($content){
    ...
    if(get_post_meta($post->ID, '_yoast_wpseo_metadesc', true)!=""){
            ...
            if(get_option('wbZ_description')!=""){
                ...
                if(strstr($desc, "\n")) {$desc=str_replace("\n", " ", $desc);}
                if(strstr($desc, "\r")) {$desc=str_replace("\r", " ", $desc);}
    ...
    if($options['fb_setting'] != "0" ){
        $fb='<a class="fb" onclick="Share.facebook(\''.$url.'\')"><i class="fa fa-facebook fa-2x"> </i></a> ';
       $btn=1;
    }
    ...
    if( $options['wa_setting'] != "0"){
        $wa='<a class="whatsapp" style="display:none;" onclick="Share.whatsapp(\''.$title.'\',\''.$url.'\')"> <i class="fa fa-whatsapp fa-2x" > </i></a> ';
        $btn=1;
    }
  ...
  if($btn==1){
    $icons='<div class="sosh-cta">'.$cta.'</div>';
...
function soshicons_displayexcerpt($excerpt) {
	$options = get_option('soshicons_options');
    $adv_options=get_option('soshicons_advanced_options');
    global $wp_query;
    $custom_fields = get_post_custom(get_query_var('p')); /* Post ID */
...
add_filter('the_excerpt', 'soshicons_displayexcerpt');
